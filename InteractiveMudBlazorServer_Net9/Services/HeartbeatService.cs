...
{
    public HeartbeatService(HttpClient client, WebAppConfig nwdConfig, ILogger<HeartbeatService> logger)
    {
        _client = client;
        _interval = nwdConfig.HeartbeatInterval;
        _logger = logger;
        Start();
    }
    ...
    public void Start()
    {
        ...
        _timer = new(async _ =>
        {
            try
            {
                var result = await _client.GetAsync("nwd/api/heartbeat");
            ...
            catch (HttpRequestException httpEx)
            {
                ...
                {
                    _logger.LogError("Something went wrong when querying for heartbeat: 404 NotFound. Are you missing the nwd server mod?");
                    IsRunning = false;
                    return;
                }
