using Interactive.Lib.Shared.Responses;
using System.Net;
...
public class ItemCacheService(ILogger<ItemCacheService> logger, HttpClient client)
{
    public async Task<bool> PopulateDictionary()
    {
        try
        {
            var result = await client.GetFromJsonAsync<GetItemsResponse>("nwd/api/items");
            if (result != null)
            {
        ...
        catch (HttpRequestException httpEx)
        {
            ...
            {
                logger.LogError("There was an error retrieving the items from the server: 404 NotFound. Are you missing the nwd server mod?");
                return false;
            }
