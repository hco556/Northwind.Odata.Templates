...
using MudBlazorWebAppTest.Extensions;
using Simple.OData.Client;
using System.Collections;
using System.Reflection;

namespace MudBlazorWebAppTest.Services
{

    public enum VersioningMode { UrlSegment, QueryString, Header }
    ...
    public class ODataClientService
    {
        ...
        public ODataClientService(string serviceBaseUrl, VersioningMode mode = VersioningMode.QueryString, string defaultVersion = "1.0")
        {
            _baseUrl = serviceBaseUrl?.TrimEnd('/') ?? throw new ArgumentNullException(nameof(serviceBaseUrl));
            _mode = mode;
            _defaultVersion = defaultVersion;

            var settings = new ODataClientSettings(new Uri(_baseUrl))
            {
                BeforeRequest = message => ApplyVersioning(message, _defaultVersion)
            };
            _client = new ODataClient(settings);
        }

        private void ApplyVersioning(HttpRequestMessage message, string apiVersion)
        {
            ...
            switch (_mode)
            {
                case VersioningMode.Header:
                    if (!message.Headers.Contains("api-version"))
                        message.Headers.Add("api-version", apiVersion);
                    break;

                case VersioningMode.QueryString:
                    var uri = message.RequestUri;
                    var sep = string.IsNullOrEmpty(uri.Query) ? "?" : "&";
                    message.RequestUri = new Uri(uri + $"{sep}api-version={Uri.EscapeDataString(apiVersion)}");
                    break;

                case VersioningMode.UrlSegment:
                    // Replace base path prefix if service expects /v{version}/resource
                    // This assumes the base URL contains a placeholder like /v{version} not present here,
                    // so we rebuild the request URI with version segment inserted after host.
                    var builder = new UriBuilder(message.RequestUri);
                    var path = builder.Path.TrimStart('/');
                    builder.Path = $"v{apiVersion}/{path}";
                    message.RequestUri = builder.Uri;
                    break;
            }
        }

        public async Task<IList<T>> GetAllAsync<T>(string entitySetName, string apiVersion = null,string ? filter = null, string ? orderBy = null, int ? top = null, int ? skip = null)
        {
            var client = _client;
            if (!string.IsNullOrEmpty(apiVersion))
            {
                // clone settings to inject per-call version
                var settings = new ODataClientSettings(new Uri(_baseUrl))
                {
                    BeforeRequest = message => ApplyVersioning(message, apiVersion)
                };
                client = new ODataClient(settings);
            }
            var query = _client.For(entitySetName);
            ...
            var entries = await client.For(entitySetName).FindEntriesAsync();
            Type type = typeof(T);
            return new List<T>();//ToObjectList(type,entries);
            
        }

        public async Task<T> GetByIdAsync<T>(string entitySetName, int key, string apiVersion = null)
        {
            var client = _client;
            if (!string.IsNullOrEmpty(apiVersion))
            {
            ...
            var entry = await client.For(entitySetName).Key(key).FindEntryAsync();
            object?  value = entry.Values.FirstOrDefault();
            Type type = typeof(T);
            if (value == null)
                return default;
            else
        ...
        public object ToObject<T>(object? entry) where T : class, new()
        {
        ...
        /// </summary>
        public IList<T> ToObjectList<T>(IEnumerable<object> entries) where T : class, new()
        {
            var list = new List<T>();
        ...
        private object MapDictionaryToType(IDictionary<string, object> dict, Type targetType)
        {
        ...
        private object MapValueToType(object value, Type targetType)
        {
        ...
        private IDictionary<string, object> ConvertToStringObjectDictionary(IDictionary genericDict)
        {
        ...
        private bool TryGetValueIgnoreCase(IDictionary<string, object> dict, string key, out object value)
        {
        ...
        private IList CreateListOfType(Type elementType)
        {
            var listType = typeof(List<>).MakeGenericType(elementType);
            return (IList)Activator.CreateInstance(listType);
        }

        private object ConvertListToPropertyType(IList listInstance, Type propertyType, Type elementType)
        {
